...
const fetchTasks = async (tab: string, page: number, filter: string, itemsPerPage: number): Promise<TasksResponse> => {
  const params = new URLSearchParams({
    tab,
    page: page.toString(),
    filter: filter === 'all' ? 'all' : filter === 'active' ? 'incomplete' : 'complete',
...
const addTaskAPI = async (data: { text: string; tab: string }): Promise<Task> => {
...
const updateTaskAPI = async (data: { id: number; text?: string; completed?: boolean }): Promise<Task> => {
